...
def validate_product_data(data: dict | None):
    ...
    try:
        ...
        return None, (
            jsonify({"error": "Entry date must use YYYY-MM-DD format."}),
            400,
        )
...
def validate_purchase_data(data: dict | None):
    ...
    if product_id is None or quantity is None:
        return None, (
            jsonify({"error": "product_id and quantity are required."}),
            400,
        )
    ...
    if product_id <= 0:
        return None, (
            jsonify({"error": "product_id must be greater than zero."}),
            400,
        )

    if quantity <= 0:
        return None, (
            jsonify({"error": "Quantity must be greater than zero."}),
            400,
        )

    return {"product_id": product_id, "quantity": quantity}, None
...
def add_product():
    ...
    try:
        product = create_product(db=db, **validated_data)
        return jsonify(
            {
...
def remove_product(product_id: int):
    ...
    try:
        ...
        return jsonify(
            {
                "error": (
                    "The product cannot be deleted because it is included "
                    "in an invoice."
                )
            }
        ), 409
...
def list_invoices():
    ...
    try:
        if g.current_user_role == "admin":
            ...
            invoices = get_user_invoices(db, g.current_user_id)
